...
for i = 1:7
    ...
    scatter3(ax6, Umap_results_filtered(idx_2, dim1), Umap_results_filtered(idx_2, dim2), Umap_results_filtered(idx_2, dim3), ...
        20, cmap(i, :), 'filled', 'MarkerFaceAlpha', 0.8);
end
...
% find indices when trial switches
trial_lengths = zeros(behavTrials.num_trials, 2);
trial = 1;
len = 1;
for i = 2:length(lick_plot_filtered)
...
scatter3(ax3, Umap_results_filtered(nonreward_idx_2, dim1), Umap_results_filtered(nonreward_idx_2, dim2), Umap_results_filtered(nonreward_idx_2, dim3), ...
    15, customRed, 'filled', 'MarkerFaceAlpha', 0.8, 'DisplayName', 'Non-Rewarded');
...
scatter3(ax3, Umap_results_filtered(reward_idx_2, dim1), Umap_results_filtered(reward_idx_2, dim2), Umap_results_filtered(reward_idx_2, dim3), ...
    15, customGreen, 'filled', 'MarkerFaceAlpha', 0.8, 'DisplayName', 'Rewarded');
...
port_7_location = 118.7;
 ...
 mode = 1; % 1 is when you just want ONE run. 0 is when you want ALL runs
 select_trials = [344, 345, 346]; % if empty it will automatically find trials based on port
 new_trials = [1; (trial_lengths(1:(length(trial_lengths)-1), 2))+1];  % Get the positions where a new trial starts

 

if ~isempty(select_trials)
    % Plots the trajectory from the lick of the first specified trial to
    % the lick of the last specified trial. This means that if three trials
    % are chosen, the trajectory will show the mouse's path and UMAP for
    % three licks and two runs (RUN 1 from lick 1 to lick 2, RUN 2 from
    % lick 2 to lick 3. i.e., it will only include two FULL trials)
    find_first_position = NaN;
    find_last_position = NaN;
    lick_locs = [new_trials(select_trials(1)+1)];
    pos_y = [];
    pos_x = [];
    for m = 1:length(select_trials)-1
        find_first_position = new_trials(select_trials(m)+1);
        find_last_position = new_trials(select_trials(m+1)+1);
        selected_trial_vector(find_first_position:find_last_position-1) = 1;
        pos_y = [pos_y, pos_y_filtered(find_first_position:find_last_position-1)];
        pos_x = [pos_x, pos_x_filtered(find_first_position:find_last_position-1)];
        lick_locs = [lick_locs; new_trials(select_trials(m)+2)];
    end
     ...
     selected_licks = Umap_results_filtered(lick_locs, :);
     selected_y = pos_y;
     selected_x = pos_x;
     ...
     subplot(4,1,1)
     hold on;
     scatter(selected_y, selected_x, new_dot, selected_location, 'filled');
     scatter(pos_y_filtered(lick_locs), pos_x_filtered(lick_locs), new_dot, customRed, 'filled');
     hold off;
     ...
     hold on;
     view(A, E);
     scatter3(selected_data(:, dim1), selected_data(:, dim2), selected_data(:, dim3), dotSize * 8, selected_location , 'filled');
     scatter3(selected_licks(:, dim1), selected_licks(:, dim2), selected_licks(:, dim3), dotSize * 8, customRed, 'filled');
     colormap('parula');
     grid('off');
     axis('tight');
     axis('off');
     hold('on');
     colorbar;
     permutation_str = strjoin(string(select_trials), ' ');
    ...
    for i = 1:size(permutations, 1)
         ...
         for start_idx = 1:(behavTrials.num_trials - window_size + 1)
             ...
             if isequal(current_window, permutation)
                 perm_found(start_idx:start_idx + window_size - 1) = 1; 
                 match_found = true;
                 if mode == 1
         ...
         for ii = 1:behavTrials.num_trials
             if perm_found(ii) == 1
                 ...
                 if idx_found_1 == 1 
                     find_first_position = new_trials(ii+1);
                     num_runs = num_runs+1;
                 ...
                 lick_locs = [lick_locs; new_trials(ii+1)];%, timestamp_beh(new_trials(ii))];
             end
         end
         
         if mode == 1
            selected_trial_vector(find_first_position:find_last_position-1) = 1;
            pos_y_filtered = pos_y_filtered(find_first_position:find_last_position-1);
            pos_x_filtered = pos_x_filtered(find_first_position:find_last_position-1);
     ...
     selected_location = pos_plot_filtered(selected_trials);
     selected_licks = Umap_results_filtered(lick_locs, :);
     selected_y = pos_y_filtered(selected_trials);
     selected_x = pos_x_filtered(selected_trials);
...
%Plot 6: Only show times of licks
% THIS PLOT DOESN"T WORK. it is trying to plot all licks, but to make this
% plot corrrectly i would need to only plot the licks of the trials where
% the mouse is running in the proper direction
